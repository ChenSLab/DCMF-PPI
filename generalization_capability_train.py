...
def train(model, optimizer, device, subgraphs_data_list_train, subgraphs_data_list_portT5_train,
          subgraphs_data_list_wavelet_train, dataset):
    # 首先构建dataloader
    all_loss = 0.0
    batch_count = 0
    all_acc_node = 0.0
    ...
    for GeoData, PorT5Date, waveletFeature in tqdm(
        ...
        # 进入两个分支模型，得到当前分支的数据
        gat_features = ConcatDic.Temporal_protein_Gat_batch(PorT5Date, Port5_Gat_model, dataset)
...
def validate(model, device, subgraphs_data_list_val, subgraphs_data_list_portT5_val, subgraphs_data_list_wavelet_val,
    ...
    all_preds = []
    all_labels = []
    set_models_mode('eval', Port5_Gat_model, MPSWA, GATE_Mechanism_Branch, VGAE_model, change_dim_wavelet)
    torch.cuda.empty_cache()  # 清理缓存，避免内存碎片化
    with torch.no_grad():
        for GeoData, PorT5Date, waveletFeature in tqdm(
            ...
            recon_edge_loss, accuracy_edge = edge_loss(model.decode_all(z),
                                                       ...
                                                       data_val.y.float())
            # 总损失
            loss = recon_node_loss + recon_edge_loss
            tqdm.write(
            f'recon_node_loss: {recon_node_loss.item():.4f}, recon_edge_loss: {recon_edge_loss.item():.4f}')
            batch_count += 1
...
# 这里只需要拿 27k的train，148k的val
def main():
    setup_logger()  # initial log
    data_splits_27k, Feature84_27k, PortT5Feature_27k = get_dataloader('27k', mode='bfs')
    data_splits_148k, Feature84_148k, PortT5Feature_148k = get_dataloader('148k', mode='bfs')

    all_branch_PortT5_data_list_train_27k = []
    all_branch_wavelet_features_list_train_27k = []
    all_branch_PortT5_data_list_val_148k = []
    all_branch_wavelet_features_list_val_148k = []

    subgraphs_data_list_train_27k, _, train_protein_ids_27k, _ = data_splits_27k
    _, subgraphs_data_list_val_148k, _, val_protein_ids_148k = data_splits_148k

    # 训练集特征处理  #27k
    for branch_protein_ids_train_27k in train_protein_ids_27k:
        branch_PortT5_data_list_train, branch_wavelet_padded_features_train = generate_subgraphs.process_protein_branch_feature(
            branch_protein_ids_train_27k, PortT5Feature_27k, Feature84_27k)
        all_branch_PortT5_data_list_train_27k.append(branch_PortT5_data_list_train)
        all_branch_wavelet_features_list_train_27k.append(branch_wavelet_padded_features_train)

    # 验证集特征处理   #148k
    for branch_protein_ids_val_148k in val_protein_ids_148k:
        branch_PortT5_data_list_val, branch_wavelet_padded_features_val = generate_subgraphs.process_protein_branch_feature(
            branch_protein_ids_val_148k, PortT5Feature_148k, Feature84_148k)
        all_branch_PortT5_data_list_val_148k.append(branch_PortT5_data_list_val)
        all_branch_wavelet_features_list_val_148k.append(branch_wavelet_padded_features_val)
    ...
    for epoch in tqdm(range(num_epochs), desc='Training'):
        ...
        train_loss, train_acc_node, train_acc_edge = train(
            ...
            subgraphs_data_list_train_27k,
            all_branch_PortT5_data_list_train_27k,
            all_branch_wavelet_features_list_train_27k, "27k"
        )
        # ================== val  ==================
        val_loss, val_acc_node, val_acc_edge, val_advanced, val_labels, val_preds = validate(
            VGAE_model, device,
            subgraphs_data_list_val_148k,
            all_branch_PortT5_data_list_val_148k,
            all_branch_wavelet_features_list_val_148k, "148k"
        )
