...
from VGAE import MLP_Ablation_experiment_PPINet
...
def train(model, optimizer, device, subgraphs_data_list_train, subgraphs_data_list_portT5_train,
    ...
    batch_count = 0
    all_acc_node = 0.0
    set_models_mode('train', Port5_Gat_model, MPSWA, GATE_Mechanism_Branch, MLP_model, change_dim_wavelet)
    for GeoData, PorT5Date, waveletFeature in tqdm(
        ...
        # 进入两个分支模型，得到当前分支的数据
        gat_features = ConcatDic.Temporal_protein_Gat_batch(PorT5Date, Port5_Gat_model)
        ...
        loss = recon_node_loss
        # 总损失
        tqdm.write(
            f'recon_node_loss: {recon_node_loss.item():.4f}')
        ...
        batch_count += 1
    return all_loss / batch_count, all_acc_node / batch_count
...
def validate(model, device, subgraphs_data_list_val, subgraphs_data_list_portT5_val, subgraphs_data_list_wavelet_val):
    ...
    all_preds = []
    all_labels = []
    set_models_mode('eval', Port5_Gat_model, MPSWA, GATE_Mechanism_Branch, MLP_model, change_dim_wavelet)
    torch.cuda.empty_cache()  # 清理缓存，避免内存碎片化
    with torch.no_grad():
        for GeoData, PorT5Date, waveletFeature in tqdm(
        ...
        return all_loss / batch_count, all_acc_node / batch_count, advanced_metrics, all_labels, all_preds
...
MLP_model = MLP_Ablation_experiment_PPINet(64, 32)

optimizer, all_params = create_optimizer_for_models(
    [Port5_Gat_model, MPSWA, GATE_Mechanism_Branch, MLP_model, change_dim_wavelet],
...
def main():
    ...
    data_splits, Feature84, PortT5Feature = get_dataloader('27k', mode='dfs')
    ...
    for epoch in tqdm(range(num_epochs), desc='Training'):
        # ================== train ==================
        train_loss, train_acc_node = train(
            MLP_model, optimizer, device,
            subgraphs_data_list_train,
            all_branch_PortT5_data_list_train,
            all_branch_wavelet_features_list_train
        )
        # ================== val  ==================
        val_loss, val_acc_node, val_advanced, val_labels, val_preds = validate(
            MLP_model, device,
        ...
        logging.info(
            f"{epoch + 1}\t"
            f"{train_loss:.4f}\t{train_acc_node:.4f}\t"
            f"{val_loss:.4f}\t{val_acc_node:.4f}\t"
        ...
        print(f"[Train] Loss: {train_loss:.4f}  Node Acc: {train_acc_node:.2%}")
        print(f"[Val]   Loss: {val_loss:.4f}  Node Acc: {val_acc_node:.2%}")
        print(
