...
def train(model, optimizer, device, subgraphs_data_list_train, subgraphs_data_list_portT5_train,
          subgraphs_data_list_wavelet_train,dataset_name):
    # 首先构建dataloader
    all_loss = 0.0
    batch_count = 0
    all_acc_node = 0.0
    ...
    for GeoData, PorT5Date, waveletFeature in tqdm(
        ...
        # 进入两个分支模型，得到当前分支的数据
        gat_features = ConcatDic.Temporal_protein_Gat_batch(PorT5Date, Port5_Gat_model,dataset_name)
...
def validate(model, device, subgraphs_data_list_val, subgraphs_data_list_portT5_val, subgraphs_data_list_wavelet_val, dataset_name, seen_proteins, val_protein_ids):
    all_loss = 0.0
    batch_count = 0
    all_acc_node = 0.0
    all_acc_edge = 0.0
    ...
    with torch.no_grad():
        for GeoData, PorT5Date, waveletFeature, protein_id_list in tqdm(
                list(zip(subgraphs_data_list_val, subgraphs_data_list_portT5_val,
                         subgraphs_data_list_wavelet_val, val_protein_ids)),
            ...
            combined_features = ConcatDic.Build_Joint_FeatureMatrix(gat_features, features_68, GATE_Mechanism_Branch)

            # 构建数据
            GeoData.x = combined_features
            GeoData.protein_ids = protein_id_list
            data_val = GeoData.to(device)
            ...
            # ========== 分类 NS/ES/BS ==========
            edge_index_np = data_val.edge_index.cpu().numpy()
            for i in range(edge_index_np.shape[1]):
                src, dst = edge_index_np[0, i], edge_index_np[1, i]
                p1, p2 = data_val.protein_ids[src], data_val.protein_ids[dst]

                pred_val = pred_np[i]
                label_val = int(label_np[i])

                seen1 = p1 in seen_proteins
                seen2 = p2 in seen_proteins

                if seen1 and seen2:
                    bs_preds.append(pred_val)
                    bs_labels.append(label_val)
                elif seen1 or seen2:
                    es_preds.append(pred_val)
                    es_labels.append(label_val)
                else:
                    ns_preds.append(pred_val)
                    ns_labels.append(label_val)
    ...
    def compute_f1(labels, preds):
        if len(labels) == 0:
            return 0.0
        return float(f1_score(labels, np.array(preds) > 0.5))

    ns_f1 = compute_f1(ns_labels, ns_preds)
    es_f1 = compute_f1(es_labels, es_preds)
    bs_f1 = compute_f1(bs_labels, bs_preds)

    total = len(ns_labels) + len(es_labels) + len(bs_labels)
    ns_ratio = len(ns_labels) / total if total > 0 else 0.0
    es_ratio = len(es_labels) / total if total > 0 else 0.0
    bs_ratio = len(bs_labels) / total if total > 0 else 0.0

    ns_es_bs_f1 = {'NS': ns_f1, 'ES': es_f1, 'BS': bs_f1}
    ns_es_bs_ratio = {'NS': ns_ratio, 'ES': es_ratio, 'BS': bs_ratio}

    return all_loss / batch_count, all_acc_node / batch_count, all_acc_edge / batch_count, \
           advanced_metrics, all_labels, all_preds, ns_es_bs_f1, ns_es_bs_ratio
...
def main():
    ...
    dataset_name ='27k'
    data_splits, Feature84, PortT5Feature = get_dataloader(dataset_name, mode='random')
    ...
    # 在 main() 中添加  这个需要进行BS.ES,NS进行统计的划分
    seen_proteins = set()
    for branch in train_protein_ids:
        seen_proteins.update(branch)
    # 训练集特征处理
    ...
    lr_decay_epoch = 20
    logging.info(
        "Epoch\tTrainLoss\tTrainAccN\tTrainAccE\tValLoss\tValAccN\tValAccE\tPrecision\tRecall\tF1\tNS\tBS\tES"
    )
    for epoch in tqdm(range(num_epochs), desc='Training'):
        ...
        # ================== val  ==================
        val_loss, val_acc_node, val_acc_edge, val_advanced, val_labels, val_preds , ns_es_bs_f1, ns_es_bs_radio= validate(
            VGAE_model, device,
        ...
        logging.info(
            ...
            f"{ns_es_bs_f1.get('NS',0):.4f}\t"
            f"{ns_es_bs_f1.get('BS',0):.4f}\t"
            f"{ns_es_bs_f1.get('ES',0):.4f}\n"
        )
        ...
        print(f"Metrics >> P: {val_advanced.get('precision', 0):.4f}  R: {val_advanced.get('recall', 0):.4f}  F1: {val_advanced.get('f1', 0):.4f}")
        print(f"ns_es_bs_f1 >> NS: {ns_es_bs_f1.get('NS', 0):.4f}  BS: {ns_es_bs_f1.get('BS', 0):.4f}  ES: {ns_es_bs_f1.get('ES', 0):.4f}")
        print(f"ns_es_bs_radio >> NS: {ns_es_bs_radio.get('NS', 0):.4f}  BS: {ns_es_bs_radio.get('BS', 0):.4f}  ES: {ns_es_bs_radio.get('ES', 0):.4f}")
