...
import StandardSelfAttentionBlock
from IntermediateConcatenate import Port5_Gat_model, GATE_Mechanism_Branch, change_dim_wavelet
...
def train(model, optimizer, device, subgraphs_data_list_train, subgraphs_data_list_portT5_train,
    ...
    for GeoData, PorT5Date, waveletFeature in tqdm(
        ...
        loss = recon_node_loss + recon_edge_loss + kl_loss
        # 总损失
        print(
...
MPSWA = StandardSelfAttentionBlock.MPSWA(84, 84).to(device)
...
def main():
    ...
    data_splits, Feature84, PortT5Feature = get_dataloader(dataset_name, mode='dfs')
